...
def main(argv):
    ...
    xs_all = np.load(os.path.join(FLAGS.logdir, "x_test.npy"))[:FLAGS.dataset_size]
    ys_all = np.load(os.path.join(FLAGS.logdir, "y_test.npy"))[:FLAGS.dataset_size]
    ...
    for path in sorted(os.listdir(os.path.join(FLAGS.logdir))):
        ...
        if not os.path.exists(os.path.join(FLAGS.logdir, path, "test_logits")):
            os.mkdir(os.path.join(FLAGS.logdir, path, "test_logits"))
        ...
        for epoch in range(first, max_epoch + 1):
            ...
            if os.path.exists(os.path.join(FLAGS.logdir, path, "test_logits", "%010d.npy" % epoch)):
            ...
            np.save(os.path.join(FLAGS.logdir, path, "test_logits", "%010d" % epoch),
                    np.array(stats)[:, None, :, :])
