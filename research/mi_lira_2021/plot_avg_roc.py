...
def load_data(root, max_model_id, target_model_id, all_targets):
    ...
    model_ids = [i for i in range(max_model_id) if i != target_model_id] + [target_model_id]

    for model_id in model_ids:

        if model_id == target_model_id and not FLAGS.target_is_reference:
...
def fig_fpr_tpr():
    ...
    for a in range(FLAGS.num_models):
        for b in range(3):
            if b == 0:
                ...
                idx = 10_000 + FLAGS.run_id * 1_000 + 100 + a
            else:
                idx = 10_000 + FLAGS.run_id * 1_000 + a
    ...
    for i in range(3):
        plt.plot(x_values, np.mean(y_values[i], 0), label=f'{16*i} points removed / target (auc={np.mean(auc_values[i], axis=0):.2f})')

    print(f'{FLAGS.run_id:03d}: {np.sum((np.array(auc_values)[0] <= np.array(auc_values)[1]) & (np.array(auc_values)[1] <= np.array(auc_values)[2]))}')
...
if __name__ == '__main__':
    ...
    flags.DEFINE_bool('target_is_reference', False, '...')
    app.run(main)
