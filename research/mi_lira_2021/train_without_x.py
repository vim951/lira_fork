...
def get_data(seed):
    ...
    # step_size = 4
    # np.random.seed(0)
    # target_idx = np.random.randint(0, 50_000, 1)[0]
    # influent_points = np.random.randint(0, 50_000, (step_size * num_gpus))
    # remove = influent_points[:(FLAGS.expid - 170) * 4]
    # keep = np.ones((50_000,)).astype('bool')
    # keep[remove] = False

    # f = glob('/mnt/LargeDisk/Victor/targeted_point_removal/to_be_removed_*.npy')[0]
    # target_idx = int(f[-4-5:-4])
    # remove = np.load(f)[:(FLAGS.expid - 170) * 4]
    # keep = np.ones((50_000,)).astype('bool')
    # keep[remove] = False
