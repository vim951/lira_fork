...
# With modifications to allow pickling the saved model.
...
def clamp_grad_l2(self, grad_input, grad_output, bound):
        g = grad_input[0]
        #self.grad_maxes.append(g.abs().max().item())
        return (l2_clip(g, bound),)
    
def clamp_grad_lweird(self, grad_input, grad_output, bound):
        g = grad_input[0]
        #self.grad_maxes.append(g.abs().max().item())
        return (lweird_clip(g, bound),)

def clamp_grad_l1(self, grad_input, grad_output, bound):
        g = grad_input[0]
        #self.grad_maxes.append(g.abs().max().item())
        return (l1_clip(g, bound),)

def clamp_input(self, input, bound):
...
def get_clamp_func(norm, bound):
    ...
    if norm == 'l2':
        return functools.partial(clamp_grad_l2, bound=bound)
    
    if norm == 'l1':
        return functools.partial(clamp_grad_l1, bound=bound)
    
    if norm == 'lweird':
        return functools.partial(clamp_grad_lweird, bound=bound)
    
    if norm == 'input':
        return functools.partial(clamp_input, bound=bound)
...
#         x.register_forward_pre_hook(clamp_input)
    ...
    for epoch in range(epochs):
        for x_batch_train, y_batch_train in train_loader:
            ...
            norms.append(next(model.parameters()).data.norm().item())

            model_optimizer.step()
        ckpts.append(deepcopy(model.state_dict()))
